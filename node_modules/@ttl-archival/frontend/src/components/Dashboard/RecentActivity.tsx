...
import { Clock, AlertTriangle, FileText, CheckCircle2, XCircle } from 'lucide-react';
...
const statusConfig = {
  ...
  Expiring: { icon: AlertTriangle, color: 'text-amber-500', bg: 'bg-amber-500/10' },
  ...
  if (loading) {
    return (
      <div className="space-y-4">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="flex items-center gap-4 p-3 rounded-xl bg-accent/20 animate-pulse">
    ...
    <div className="space-y-4 relative z-10">
      {activities.map((activity) => {
        ...
        return (
          ...
          >
            <div className={cn("w-10 h-10 rounded-lg flex items-center justify-center shrink-0 transition-transform group-hover:scale-110", config.bg, config.color)}>
