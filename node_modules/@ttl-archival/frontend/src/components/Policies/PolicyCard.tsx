...
import { ShieldCheck, Clock, Database, MoreVertical, ArrowUpRight, Trash2, Edit2 } from 'lucide-react';
    ...
    <div className="p-6 rounded-3xl bg-card border border-border/50 hover:border-primary/20 hover:shadow-2xl hover:shadow-primary/5 transition-all duration-300 group cursor-default relative overflow-hidden flex flex-col">
      ...
      <div className="flex items-center gap-4 mb-6">
        <div className={cn(
          ...
          policy.status === 'Active' ? 'text-primary' : 'text-muted-foreground'
        )}>
          <ShieldCheck className="h-6 w-6" />
        </div>
        <div>
          <h4 className="font-bold text-sm tracking-tight">{policy.name}</h4>
          <div className={cn("px-2 py-0.5 rounded-full text-[10px] font-bold mt-1 inline-block border", statusColors[policy.status])}>
            {policy.status}
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4 flex-1">
        ...
        <div className="flex flex-wrap gap-2 pt-2">
          {policy.compression_enabled && (
             <span className="text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-lg bg-blue-500/10 text-blue-500 border border-blue-500/20">lz4-comp</span>
          )}
          {policy.encryption_enabled && (
             <span className="text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-lg bg-purple-500/10 text-purple-500 border border-purple-500/20">aes-256</span>
          )}
          {policy.auto_cleanup && (
             <span className="text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-lg bg-amber-500/10 text-amber-500 border border-amber-500/20">auto-flush</span>
          )}
        </div>
      </div>
