...
const policySchema = z.object({
  ...
  ttl_days: z.number().min(1, 'Retention period must be at least 1 day').max(3650, 'Max retention is 10 years'),
...
export function PolicyForm({ policy, onSubmit, onCancel, isSubmitting }: PolicyFormProps) {
  const {
    register,
    handleSubmit,
    setValue,
      ...
      <div className="w-full max-w-2xl bg-card border border-border/50 rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
        <div className="flex items-center justify-between p-8 border-b border-border/30 bg-accent/5">
          <div className="flex flex-col">
            ...
            <p className="text-sm text-muted-foreground mt-1">Configure retention rules and storage settings.</p>
          </div>
          <button 
            onClick={onCancel}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-8 space-y-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                />
                {errors.description && <p className="text-[10px] text-rose-500 font-bold ml-2">{errors.description.message}</p>}
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                />
                {errors.ttl_days && <p className="text-[10px] text-rose-500 font-bold ml-2">{errors.ttl_days.message}</p>}
              </div>
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <option value="Glacier Deep">Amazon Glacier Deep Archive</option>
                  <option value="Google Archive">Google Cloud Archive</option>
                  <option value="Azure Blob">Azure Blob Archive Tier</option>
                </select>
                {errors.storage_location && <p className="text-[10px] text-rose-500 font-bold ml-2">{errors.storage_location.message}</p>}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-6 bg-accent/10 rounded-3xl border border-border/20">
             <div className="flex items-center gap-3">
                <input type="checkbox" {...register('compression_enabled')} className="w-5 h-5 rounded-lg accent-primary cursor-pointer" />
                <label className="text-xs font-bold text-muted-foreground">LZ4 Compression</label>
             </div>
             <div className="flex items-center gap-3">
                <input type="checkbox" {...register('encryption_enabled')} className="w-5 h-5 rounded-lg accent-primary cursor-pointer" />
                <label className="text-xs font-bold text-muted-foreground">AES-256 Encryption</label>
             </div>
             <div className="flex items-center gap-3">
                <input type="checkbox" {...register('auto_cleanup')} className="w-5 h-5 rounded-lg accent-primary cursor-pointer" />
                <label className="text-xs font-bold text-muted-foreground">Auto-Cleanup</label>
             </div>
             <div className="flex items-center gap-3">
                <select {...register('status')} className="bg-transparent text-[10px] font-bold text-primary focus:outline-none cursor-pointer">
