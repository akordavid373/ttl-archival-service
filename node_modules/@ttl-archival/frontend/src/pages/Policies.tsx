...
export function Policies() {
  ...
  const [editingPolicy, setEditingPolicy] = useState<ArchivalPolicy | undefined>()
      ...
      if ((import.meta as any).env.DEV) {
        const mockPolicies: ArchivalPolicy[] = [
          { 
            id: '1', 
            name: 'User Data TTL', 
            description: 'Automated archival for user profile data and activity logs.',
            ttl_days: 30, 
            status: 'Active', 
          ...
          { 
            ...
            description: 'Long term retention for promotional media and assets.',
            ttl_days: 365, 
            status: 'Inactive', 
  ...
  const handleCreateOrUpdate = async (data: PolicyFormData) => {
    try {
      if (editingPolicy) {
        ...
        setPolicies((prev: ArchivalPolicy[]) => prev.map((p: ArchivalPolicy) => p.id === editingPolicy.id ? { ...p, ...data, updated_at: new Date().toISOString() } : p))
        addNotification({
      ...
      if ((import.meta as any).env.DEV) {
        if (editingPolicy) {
           setPolicies((prev: ArchivalPolicy[]) => prev.map((p: ArchivalPolicy) => p.id === editingPolicy.id ? { ...p, ...data } : p))
        } else {
           setPolicies((prev: ArchivalPolicy[]) => [{ ...data, id: Date.now().toString(), archives_count: 0, created_at: '', updated_at: ''} as ArchivalPolicy, ...prev])
        }
        setIsFormOpen(false)
        setEditingPolicy(undefined)
      } else {
        addNotification({
          ...
          message: 'An error occurred while saving the policy. Please verify your data.',
          type: 'error',
          timestamp: new Date()
        })
      }
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Are you sure you want to permanently delete this policy? This action cannot be undone.')) return

    try {
      await axios.delete(`${API_BASE_URL}/policies/${id}`)
      setPolicies((prev: ArchivalPolicy[]) => prev.filter((p: ArchivalPolicy) => p.id !== id))
      addNotification({
      ...
      <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground flex items-center gap-2">
            ...
            Manage rules, TTL settings, and automated archival workflows for all data nodes.
          </p>
        </div>
        <button 
             ...
             <div className="mt-8 pt-8 border-t border-border/20 space-y-4">
                <div className="space-y-2">
                   <div className="flex items-center justify-between text-xs">
                      <span className="text-muted-foreground font-medium">Compliance Coverage</span>
                ...
                <div className="space-y-2">
                   <div className="flex items-center justify-between text-xs">
                      <span className="text-muted-foreground font-medium">Auto-Archival Precision</span>
           ...
           <div className="p-6 bg-primary/5 rounded-[2rem] border border-primary/10">
              <div className="flex gap-4">
                ...
                <div className="space-y-1">
                   <h4 className="font-bold text-sm tracking-tight text-primary">Optimization Tip</h4>
                   <p className="text-[11px] leading-relaxed text-muted-foreground">Policies with TTL &lt; 7 days are processed in prioritized cleanup cycles to maximize storage efficiency.</p>
                </div>
              </div>
           </div>
        </div>
