...
export function AdvancedFileUpload({
          ...
          prev.map((f: UploadingFile) =>
            f.id === id
              ? {
                  ...
                  uploadSpeed,
                  timeRemaining:
                    timeRemaining > 0 ? Math.round(timeRemaining) : 0,
                }
              : f,
          ),
        );
  ...
  const retryFailedUploads = () => {
    const failedFiles = files.filter(
      (f: UploadingFile) => f.status === "error",
    );
    failedFiles.forEach((f) => {
      setFiles((prev: UploadingFile[]) =>
        prev.map((file: UploadingFile) =>
          file.id === f.id
      ...
      >
        ...
        <div className="text-center">
          ...
          <p className="text-xs text-muted-foreground mt-2 font-medium">
            Supports {allowedTypes.join(", ")} (Max {formatSize(maxSize)},{" "}
            {maxFiles} files)
          </p>
        </div>
