...
import { Wallet, LogOut, ShieldCheck, Globe, Link2, AlertCircle } from 'lucide-react';
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <div className={cn(
              ...
              wallet?.connected ? "bg-emerald-500/10 text-emerald-500 shadow-emerald-500/10" : "bg-primary/10 text-primary shadow-primary/10"
            )}>
              <Wallet className="h-6 w-6" />
            </div>
            <div>
              <h3 className="text-xl font-bold tracking-tight">Identity Provider</h3>
              <p className="text-[10px] text-muted-foreground uppercase font-extrabold tracking-widest mt-1">Freighter Wallet Connection</p>
            </div>
          </div>
        ...
        {!wallet?.connected ? (
          <div className="space-y-6">
            <p className="text-sm text-muted-foreground leading-relaxed">
              Connect your Stellar wallet to authorize archival operations and manage on-chain storage policies securely.
            </p>
            <button 
              onClick={onConnect}
              disabled={isLoading}
          ...
          <div className="space-y-6">
            <div className="p-5 rounded-2xl bg-accent/20 border border-border/40 space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest leading-none">Wallet Address</span>
            ...
            <div className="grid grid-cols-2 gap-4">
               <div className="p-4 rounded-2xl bg-accent/10 border border-border/20">
                  <p className="text-[10px] font-bold text-muted-foreground uppercase opacity-60 mb-1">Network</p>
                  <p className="text-xs font-extrabold uppercase tracking-widest text-primary">{wallet.network}</p>
               </div>
               <div className="p-4 rounded-2xl bg-accent/10 border border-border/20">
                  <p className="text-[10px] font-bold text-muted-foreground uppercase opacity-60 mb-1">Status</p>
                  <p className="text-xs font-extrabold text-emerald-500">Authenticated</p>
               </div>
            </div>

            <div className="pt-2 flex gap-3">
              <button 
                onClick={() => onNetworkChange(wallet.network === 'testnet' ? 'public' : 'testnet')}
        ...
        <div className="mt-8 pt-8 border-t border-border/20">
           <div className="flex items-start gap-3 p-4 rounded-2xl bg-amber-500/5 border border-amber-500/10">
              ...
              <p className="text-[10px] font-bold text-amber-600/80 leading-relaxed uppercase tracking-tight">
                All on-chain operations incur a network fee in XLM. Ensure your wallet is funded.
              </p>
           </div>
        </div>
      </div>
