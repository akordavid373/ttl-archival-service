...
export function InteractiveMap({
  ...
  const reverseGeocode = useCallback(
    async (latlng: LatLng): Promise<string> => {
      try {
        const response = await fetch(
        ...
        {allowRoutePlanning && (
          <div className="bg-white shadow-md rounded-lg p-2 flex gap-2">
            ...
            <Button
              ...
              disabled={
                !markers.find((m) => m.type === "start") ||
                !markers.find((m) => m.type === "end")
              }
              className="bg-white"
            >
        ...
        <div className="absolute bottom-4 left-4 z-[1000] bg-white shadow-md rounded-lg p-3">
          {routes.find((r) => r.id === selectedRoute) && (
            <div className="text-sm">
              ...
              <p className="text-gray-600">
                Duration: {routes.find((r) => r.id === selectedRoute)?.duration}{" "}
                min
              </p>
            </div>
          )}
