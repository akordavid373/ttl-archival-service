...
import { Archive as ArchiveIcon, Plus, RefreshCw, X } from "lucide-react";
...
export function Archives() {
  ...
  const [selectedArchive, setSelectedArchive] =
    useState<DetailedArchiveEntry | null>(null);
  ...
  const fetchArchives = useCallback(
    async (isSilent = false, append = false) => {
      ...
      try {
        ...
        if ((import.meta as any).env.DEV) {
          ...
          const mockItems: DetailedArchiveEntry[] = Array(pageSize)
            .fill(0)
            .map((_, i) => ({
  ...
  const deleteArchive = async (id: string) => {
    if (
      !window.confirm(
        "IRREVERSIBLE ACTION: Are you sure you want to permanently prune this archive metadata? Data on Stellar remains immutable.",
      )
    )
      return;
    ...
    try {
      ...
      setArchives((prev: DetailedArchiveEntry[]) =>
        prev.filter((a: DetailedArchiveEntry) => a.id !== id),
      );
      if (selectedArchive?.id === id) setSelectedArchive(null);
      addNotification({
        status: "Metadata Purged",
        message: `Archive ${id} record has been safely removed.`,
        type: "info",
        timestamp: new Date(),
      } as any);
    } catch (err) {
      ...
      if ((import.meta as any).env.DEV) {
        ...
        addNotification({
          title: "System Error",
          message: "Failed to prune archive record.",
          type: "error",
          timestamp: new Date(),
        });
      }
  ...
  const downloadArchive = (archive: DetailedArchiveEntry) => {
    ...
    setTimeout(() => {
      window.alert(
        `Simulating secure download for ${archive.filename} via high-availability node.`,
      );
    }, 1000);
  };
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1">
          <h2 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-primary to-primary/60 bg-clip-text text-transparent">
            Archive Record Browser
          </h2>
          <p className="text-muted-foreground flex items-center gap-2">
            <ArchiveIcon className="h-4 w-4" />
            Auditing global retention nodes and cryptographically anchored
            storage.
          </p>
        </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6 bg-background/80 backdrop-blur-xl animate-in fade-in duration-300 overflow-y-auto">
          <div className="bg-card w-full max-w-2xl rounded-[2.5rem] border border-border/50 shadow-2xl overflow-hidden relative animate-in zoom-in-95 duration-300 my-8">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-2xl font-bold tracking-tight leading-none">
                    Initialize Archival Pack
                  </h3>
                  <p className="text-[10px] text-muted-foreground mt-2 uppercase font-extrabold tracking-widest">
                    Dataset Chain Validation
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-8 bg-accent/10 border-t border-border/30 flex justify-end gap-3">
              <button
                onClick={() => setIsUploadOpen(false)}
                className="px-6 py-3 text-xs font-bold hover:bg-accent rounded-2xl transition-colors"
              >
                Discard Draft
              </button>
              <button className="px-8 py-3 bg-primary text-primary-foreground font-bold text-xs rounded-2xl shadow-lg shadow-primary/20 hover:scale-[1.05] transition-all">
                Proceed to Encrypt & Anchor
              </button>
            </div>
          </div>
        </div>
