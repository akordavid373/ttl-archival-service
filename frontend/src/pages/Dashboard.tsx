...
export function Dashboard() {
  ...
  return (
    <div className="space-y-8">
      <SEO
        title="Dashboard"
        description="Overview of your archival status and statistics."
      />
      ...
      <div className="p-8 rounded-3xl bg-card border border-border/50 shadow-sm relative overflow-hidden group">
        <div className="flex items-center justify-between mb-8">
          ...
          <p className="text-xs text-muted-foreground">
            Visualizing data flow through archival tiers
          </p>
        </div>
        <RetentionSankey />
      </div>
      ...
      <div className="p-8 rounded-3xl bg-card border border-border/50 shadow-sm relative overflow-hidden group">
        ...
        <div className="space-y-4 relative z-10">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="flex items-center gap-4 p-3 rounded-xl hover:bg-accent/50 transition-colors cursor-pointer border border-transparent hover:border-border/40"
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate group-hover:text-primary transition-colors">
                  user_logs_2024_03_2{i}.sql
                </p>
                <p className="text-[10px] text-muted-foreground flex items-center gap-1">
                  <Clock className="h-3 w-3" />2 hours ago · 14.2 MB
                </p>
              </div>
            </div>
          ))}
