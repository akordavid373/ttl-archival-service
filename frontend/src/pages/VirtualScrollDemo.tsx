...
import { Scroll, Database, Zap, Accessibility, BarChart3, Settings, SkipForward } from 'lucide-react'
...
export function VirtualScrollDemo() {
  ...
  const generateItems = (count: number): DemoItem[] => {
    const categories = ['Technology', 'Business', 'Design', 'Marketing', 'Sales', 'Support']
  ...
  // Render custom item
  const renderItem = (item: VirtualDemoItem, index: number, isVisible: boolean) => {
    ...
    return (
      <div className={`p-4 border-b border-gray-200 hover:bg-gray-50 transition-all duration-200 ${
        isVisible ? 'opacity-100 transform translate-y-0' : 'opacity-0 transform translate-y-2'
      }`}>
        <div className="flex items-start justify-between">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-2">
              <h3 className="font-semibold text-gray-900 truncate">{demoItem.title}</h3>
              <span className={`px-2 py-1 rounded text-xs font-medium ${priorityColors[demoItem.priority]}`}>
                {demoItem.priority}
              </span>
              <span className={`px-2 py-1 rounded text-xs font-medium ${categoryColors[demoItem.category as keyof typeof categoryColors]}`}>
                {demoItem.category}
              </span>
            </div>
          ...
          <div className="ml-4 flex-shrink-0">
            <div className={`w-3 h-3 rounded-full ${
              demoItem.priority === 'high' ? 'bg-red-500' : 
              demoItem.priority === 'medium' ? 'bg-yellow-500' : 'bg-gray-400'
            }`} />
          </div>
        </div>
      </div>
  ...
  const performanceMetrics = useMemo(() => {
    ...
    const memorySavings = totalItems > 100 ? ((totalItems - visibleItems) / totalItems * 100).toFixed(1) : '0'
  ...
  const handleAddItems = () => {
    ...
    const newVirtualItems: VirtualDemoItem[] = newDemoItems.map((item, index) => ({
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="text-center space-y-2">
          <h1 className="text-4xl font-bold text-gray-900">Virtual Scroll Demo</h1>
          <p className="text-lg text-gray-600">
            High-performance virtual scrolling for large datasets with memory optimization
          </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Button size="sm" variant="outline" onClick={handleRemoveItems}>
                    -100
                  </Button>
                </div>
              </div>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-2xl font-bold text-blue-600">{performanceMetrics.totalItems.toLocaleString()}</div>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-2xl font-bold text-green-600">{performanceMetrics.visibleItems}</div>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-2xl font-bold text-yellow-600">{performanceMetrics.memorySavings}%</div>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-2xl font-bold text-purple-600">{performanceMetrics.renderTime}</div>
          ...
          <CardHeader>
            <div className="flex justify-between items-center">
              <div>
                ...
                <CardDescription>
                  Scroll through {items.length.toLocaleString()} items with optimal performance
                </CardDescription>
              </div>
              <div className="flex items-center gap-2">
                <span className={`px-2 py-1 rounded text-xs font-medium ${
                  dynamicHeight ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'
                }`}>
                  {dynamicHeight ? 'Dynamic Heights' : 'Fixed Heights'}
                </span>
                <span className={`px-2 py-1 rounded text-xs font-medium ${
                  horizontal ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'
                }`}>
                  {horizontal ? 'Horizontal' : 'Vertical'}
                </span>
              </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <Card>
              ...
              <CardContent>
                <p className="text-sm text-gray-600">
                  Handles thousands of items without performance degradation. Only renders visible items for optimal performance.
                </p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <p className="text-sm text-gray-600">
                  Efficient memory usage by recycling DOM elements and only keeping visible items in memory.
                </p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <p className="text-sm text-gray-600">
                  Full keyboard navigation, screen reader support, and ARIA labels for complete accessibility.
                </p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <p className="text-sm text-gray-600">
                  Support for items with varying heights using ResizeObserver API for automatic size detection.
                </p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <p className="text-sm text-gray-600">
                  Works both vertically and horizontally with the same performance optimizations.
                </p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <p className="text-sm text-gray-600">
                  Intelligent scroll detection, direction tracking, and smooth scrolling with debouncing.
                </p>
              </CardContent>
            </Card>
          </div>
