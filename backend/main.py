...
from .middleware.version_middleware import (
...
async def create_policy(
    policy: ArchivePolicyCreate, request: Request, db: Session = Depends(get_db)
):
...
async def create_archive_record(
    record: ArchiveRecordCreate, request: Request, db: Session = Depends(get_db)
):
...
async def list_archives(
    params: ArchiveListParams = None, db: Session = Depends(get_db)
):
    ...
    policy_id = (
        int(params.policy_id)
        if params and hasattr(params, "policy_id") and params.policy_id
        else None
    )
    ...
    min_size = (
        int(params.min_size)
        if params and hasattr(params, "min_size") and params.min_size
        else None
    )
    max_size = (
        int(params.max_size)
        if params and hasattr(params, "max_size") and params.max_size
        else None
    )
    sort_field = (
        params.sort_field if params and hasattr(params, "sort_field") else "created_at"
    )
    sort_order = (
        params.sort_order if params and hasattr(params, "sort_order") else "desc"
    )
...
async def get_archive_record(record_id: int, db: Session = Depends(get_db)):
...
async def update_settings(
    settings_update: UserSettingsUpdate, request: Request, db: Session = Depends(get_db)
):
    ...
    try:
        result = await settings_service.update_settings(
            db, settings_update.dict(exclude_unset=True)
        )
...
async def get_api_version():
    ...
    return {"version": version_manager.get_latest_version(), "status": "active"}
